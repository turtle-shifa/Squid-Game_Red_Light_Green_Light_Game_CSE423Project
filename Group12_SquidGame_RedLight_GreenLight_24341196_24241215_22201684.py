...
# Sky and cloud variables
SKY_RADIUS = 2000
NUM_CLOUDS = 15
cloud_positions = []

# Doll-related variables
DOLL_POSITION = (0, 0, -980)
DOLL_HEIGHT = 175
DOLL_HEAD_RADIUS = 42
DOLL_BODY_WIDTH = 70
DOLL_BODY_HEIGHT = 112
DOLL_BODY_DEPTH = 35
...
TREE_TYPES = ["pine"]
NUM_TREES = 20  # Number of trees on each side

# Arrays to store tree positions
left_tree_positions = []
right_tree_positions = []

GUARD_POSITIONS = [(DOLL_POSITION[0] - 150, 0, DOLL_POSITION[2] + 50),(DOLL_POSITION[0] + 150, 0, DOLL_POSITION[2] + 50)]

def draw_guard(position, mirror=False):
    x, y, z = position
    ...
    if mirror:
        glScalef(-1, 1, 1)  # Mirror for right side positioning
    ...
    glColor3f(1, 0, 0)  # Red
    glPushMatrix()
    glTranslatef(0, 140, 15)
    glScalef(50, 15, 10)
    glutSolidCube(1.0)
    glPopMatrix()
    
    # Guard's arms - black and crossed in front
    glColor3f(0, 0, 0)  # Black
    
    # Left arm
    glPushMatrix()
    glTranslatef(-15, 80, 20)
    glRotatef(30, 0, 0, 1)
    glScalef(10, 40, 15)
    glutSolidCube(1.0)
    glPopMatrix()
...
def draw_guards():
    # Draw left guard
    draw_guard(GUARD_POSITIONS[0], mirror=False)
...
def draw_bullet():
    """Draw the red bullet if it's active"""
    if bullet_active and bullet_pos:
        glPushMatrix()
        glColor3f(1.0, 0.0, 0.0)  # Bright red color
        glTranslatef(bullet_pos[0], bullet_pos[1], bullet_pos[2])
        
        # Create a glowing effect with a larger transparent sphere around the bullet
        glPushMatrix()
        glColor4f(1.0, 0.0, 0.0, 0.3)  # Semi-transparent red
        gluSphere(gluNewQuadric(), bullet_size * 1.5, 16, 16)
        glPopMatrix()
        
        # Solid core of the bullet
        glColor3f(1.0, 0.2, 0.0)
        gluSphere(gluNewQuadric(), bullet_size, 16, 16)
...
def handle_player_death():
    """
    Handles the death animation when player is eliminated:
    - Makes the player fall/lie down
    - Creates a red bullet that shoots from doll's eye
    """
    global player_falling, player_rotation, bullet_pos, bullet_active, player_alive
    
    # If player just died and death animation hasn't started yet
    if not player_alive and not player_falling and not bullet_active:
        player_falling = True
        player_rotation = 0
        
        # Create bullet from doll's eye
        # Calculate the bullet starting position (from doll's right eye)
        eye_x = DOLL_POSITION[0] + 20 * math.sin(math.radians(doll_rotation))
        eye_y = DOLL_POSITION[1] + DOLL_BODY_HEIGHT + DOLL_HEAD_RADIUS/2 + 5
        eye_z = DOLL_POSITION[2] + DOLL_HEAD_RADIUS * math.cos(math.radians(doll_rotation))
        
        # Set the target as the player's position
        target_x = player_pos[0]
        target_y = player_pos[1] + player_height/2
        target_z = player_pos[2]
        
        # Calculate direction vector from eye to player
        dir_x = target_x - eye_x
        dir_y = target_y - eye_y
        dir_z = target_z - eye_z
        ...
        bullet_pos = [eye_x, eye_y, eye_z, dir_x, dir_y, dir_z]
        bullet_active = True
    
    # Update falling animation for player
    if player_falling and player_rotation < 90:
        player_rotation += 5  # Rotate 5 degrees per frame until reaching 90 degrees
    
    # Update bullet position if active
    if bullet_active and bullet_pos:
        # Move the bullet in the direction vector
        bullet_pos[0] += bullet_pos[3] * bullet_speed
        bullet_pos[1] += bullet_pos[4] * bullet_speed
        bullet_pos[2] += bullet_pos[5] * bullet_speed
        
        # Check if bullet has reached player
        dist_x = bullet_pos[0] - player_pos[0]
        dist_y = bullet_pos[1] - (player_pos[1] + player_height/2)
        dist_z = bullet_pos[2] - player_pos[2]
        
        distance = math.sqrt(dist_x**2 + dist_y**2 + dist_z**2)
        
        # If bullet reached player or went too far, deactivate it
        if distance < 20 or distance > 2000:
            bullet_active = False

def check_game_conditions():
    global player_alive, player_won, was_moving_during_red, game_active, player_falling
...
def draw_text(x, y, text, font=GLUT_BITMAP_HELVETICA_18):
    glColor3f(0.1, 0.1, 0.5)  # Vibrant dark blue for all text
...
def draw_traffic_light():
    light_x = -150
    light_y = DOLL_HEIGHT + 100
    light_z = DOLL_POSITION[2] + 50
    
    glColor3f(0.3, 0.3, 0.3) 
    glPushMatrix()
    glTranslatef(light_x, light_y/2, light_z)
    glScalef(10, light_y, 10)
    glutSolidCube(1.0)
    glPopMatrix()
    ...
    if is_red_light:
        ...
        glColor3f(0.5, 0.0, 0.0)  # Darker red
...
def draw_doll():
    ...
    glPushMatrix()
    glTranslatef(x, y, z)
    glRotatef(doll_rotation, 0, 1, 0)
    
    # doll's body
    glColor3f(1, 0.318, 0)
    glPushMatrix()
    glTranslatef(0, DOLL_BODY_HEIGHT/2, 0)
    glScalef(DOLL_BODY_WIDTH, DOLL_BODY_HEIGHT, DOLL_BODY_DEPTH)
    glutSolidCube(1.0)
    glPopMatrix()
    
    # doll's head
    glColor3f(0.969, 0.863, 0.863)
    glPushMatrix()
...
def draw_environment():
    # Draw the main path from start to finish with a green gradient
    glBegin(GL_QUADS)
    # Bottom (closer to player) - darker green
    glColor3f(0.13, 0.55, 0.13)
    glVertex3f(-PATH_WIDTH/2, 0, START_LINE_Z)
    glVertex3f(PATH_WIDTH/2, 0, START_LINE_Z)
    # Top (closer to doll) - lighter green
    glColor3f(0.4, 0.85, 0.4)
    glVertex3f(PATH_WIDTH/2, 0, FINISH_LINE_Z)
    glVertex3f(-PATH_WIDTH/2, 0, FINISH_LINE_Z)
    glEnd()

    # Add subtle semi-transparent stripes for a sporty look
    glEnable(GL_BLEND)
    glBlendFunc(GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA)
    stripe_count = 12
    stripe_width = PATH_WIDTH / stripe_count
    for i in range(stripe_count):
        if i % 2 == 0:
            glColor4f(1.0, 1.0, 1.0, 0.07)  # Very subtle white stripe
            x1 = -PATH_WIDTH/2 + i * stripe_width
            x2 = x1 + stripe_width
            glBegin(GL_QUADS)
            glVertex3f(x1, 0.1, START_LINE_Z)
            glVertex3f(x2, 0.1, START_LINE_Z)
            glVertex3f(x2, 0.1, FINISH_LINE_Z)
            glVertex3f(x1, 0.1, FINISH_LINE_Z)
            glEnd()
    glDisable(GL_BLEND)
    ...
    # Draw trees on both sides of the path
    for tree_pos in left_tree_positions:
        x, z, tree_type, scale, pine_color = tree_pos
        draw_tree(x, z, tree_type, scale, pine_color)
    for tree_pos in right_tree_positions:
        x, z, tree_type, scale, pine_color = tree_pos
        draw_tree(x, z, tree_type, scale, pine_color)
...
def update_doll_rotation():
    ...
    if is_rotating:
        ...
        if abs(target_rotation - doll_rotation) < rotation_step:
            ...
            angle_diff = (target_rotation - doll_rotation) % 360
...
def display_game_status():
    ...
    if countdown_started:
        ...
        glColor3f(0.1, 0.1, 0.5)  # Vibrant dark blue
...
def draw_flat_background():
    """Draw a flat blue sky, 2D clouds, and simple background trees/bushes."""
    # Draw sky (flat blue rectangle)
    glMatrixMode(GL_PROJECTION)
    glPushMatrix()
    glLoadIdentity()
    ...
    glColor3f(1, 1, 1)
    ellipse(200, 700, 70, 30)
    ellipse(250, 730, 40, 18)
    ellipse(800, 750, 60, 25)
    ellipse(600, 720, 50, 20)
    ellipse(400, 780, 30, 12)
...
def draw_shapes():
    ...
    draw_flat_background()  # Draw 2D background first
    draw_environment()
    draw_doll()
    draw_player()
    draw_traffic_light()
...
def initialize_trees():
    """Generate fixed positions for trees on both sides of the path, with stable color for each tree"""
    global left_tree_positions, right_tree_positions
    
    # Only generate trees if the arrays are empty
    if left_tree_positions and right_tree_positions:
        return
    ...
    # Space between START_LINE_Z and FINISH_LINE_Z
    path_length = abs(START_LINE_Z - FINISH_LINE_Z)
    
    # Distance between trees (with some randomness)
    distance = path_length / (NUM_TREES * 0.8)
    
    # Generate tree positions for left side
    for i in range(NUM_TREES):
        z_pos = START_LINE_Z - i * distance - random.uniform(0, distance * 0.5)
        if z_pos < FINISH_LINE_Z:
            break
        x_pos = -PATH_WIDTH/2 - random.uniform(50, 150)
        tree_type = random.choice(TREE_TYPES)
        scale = random.uniform(0.8, 1.5)
        # Assign a stable color for pine trees
        if tree_type == "pine":
            pine_color = (
                0.0 + random.uniform(0, 0.08),
                0.4 + random.uniform(-0.05, 0.08),
                0.0
            )
        else:
            pine_color = None
        left_tree_positions.append((x_pos, z_pos, tree_type, scale, pine_color))
    
    # Generate tree positions for right side
    for i in range(NUM_TREES):
        z_pos = START_LINE_Z - i * distance - random.uniform(0, distance * 0.5)
        if z_pos < FINISH_LINE_Z:
            break
        ...
        if tree_type == "pine":
            ...
            pine_color = None
        right_tree_positions.append((x_pos, z_pos, tree_type, scale, pine_color))

def draw_tree(x, z, tree_type="pine", scale=1.0, pine_color=None):
    """Draw a more appealing tree at the specified position with the given type and scale"""
    trunk_height = TREE_HEIGHT * 0.4 * scale
    trunk_radius = TREE_TRUNK_RADIUS * scale
    crown_radius = TREE_CROWN_RADIUS * scale
    crown_height = TREE_HEIGHT * 0.6 * scale

    glPushMatrix()
    glTranslatef(x, 0, z)

    # Draw trunk with slight color variation
    if tree_type == "pine":
        glColor3f(0.5, 0.27, 0.07)
    elif tree_type == "oak":
        glColor3f(0.4 + random.uniform(-0.05, 0.05), 0.2, 0.0)
    glPushMatrix()
    glRotatef(-90, 1, 0, 0)
    gluCylinder(gluNewQuadric(), trunk_radius, trunk_radius * 0.8, trunk_height, 12, 4)
    glPopMatrix()

    # Draw tree crown with more detail and color variation
    if tree_type == "pine":
        # Use the stable color for all branches
        if pine_color is None:
            pine_color = (0.0, 0.4, 0.0)
        for i in range(4):
            glColor3f(*pine_color)
            glPushMatrix()
            glTranslatef(0, trunk_height + i * crown_height * 0.22, 0)
            glRotatef(-90, 1, 0, 0)
            cone_height = crown_height * (1.0 - i * 0.18)
            cone_radius = crown_radius * (1.0 - i * 0.13)
            glutSolidCone(cone_radius, cone_height, 16, 8)
            glPopMatrix()
    elif tree_type == "oak":
        # Irregular oak crown using multiple cones and cylinders for a non-circular look
        for i in range(3):
            glColor3f(0.1 + random.uniform(0, 0.08), 0.5 + random.uniform(-0.05, 0.08), 0.1)
            glPushMatrix()
            glTranslatef(random.uniform(-crown_radius*0.2, crown_radius*0.2), trunk_height + crown_height * 0.7 + random.uniform(-crown_radius*0.1, crown_radius*0.1), random.uniform(-crown_radius*0.2, crown_radius*0.2))
            glRotatef(random.uniform(-30, 30), 0, 1, 0)
            glScalef(1.0 + random.uniform(-0.2, 0.2), 0.7 + random.uniform(-0.1, 0.1), 1.0 + random.uniform(-0.2, 0.2))
            glutSolidCone(crown_radius * random.uniform(0.7, 1.0), crown_height * random.uniform(0.5, 0.8), 10, 6)
            glPopMatrix()
        for i in range(2):
            glColor3f(0.1 + random.uniform(0, 0.08), 0.5 + random.uniform(-0.05, 0.08), 0.1)
            glPushMatrix()
            glTranslatef(random.uniform(-crown_radius*0.2, crown_radius*0.2), trunk_height + crown_height * 0.7 + random.uniform(-crown_radius*0.1, crown_radius*0.1), random.uniform(-crown_radius*0.2, crown_radius*0.2))
            glRotatef(90, 1, 0, 0)
            gluCylinder(gluNewQuadric(), crown_radius * random.uniform(0.2, 0.4), crown_radius * random.uniform(0.1, 0.3), crown_height * random.uniform(0.3, 0.5), 8, 4)
            glPopMatrix()
...
def initialize_clouds():
    ...
    for _ in range(NUM_CLOUDS):
        ...
        x = random.uniform(-SKY_RADIUS, SKY_RADIUS)
        y = random.uniform(300, 800)  # Height range for clouds
        z = random.uniform(FINISH_LINE_Z - 500, START_LINE_Z + 500)
        size = random.uniform(100, 200)  # Random cloud size
        cloud_positions.append((x, y, z, size))

def draw_sky():
    """Draw the sky dome"""
    glPushMatrix()
    
    # Draw sky dome (hemisphere)
    glColor3f(0.529, 0.808, 0.922)  # Sky blue color
    glPushMatrix()
    glRotatef(90, 1, 0, 0)  # Rotate to make hemisphere face up
    glutSolidSphere(SKY_RADIUS, 32, 32)  # Draw hemisphere
    glPopMatrix()
    
    glPopMatrix()

def draw_cloud(x, y, z, size):
    """Draw a single cloud at the specified position"""
    glPushMatrix()
    glTranslatef(x, y, z)
    glColor3f(1.0, 1.0, 1.0)  # White color for clouds
    
    # Draw multiple spheres to create a fluffy cloud
    num_spheres = 5
    for i in range(num_spheres):
        offset_x = random.uniform(-size/3, size/3)
        offset_y = random.uniform(-size/4, size/4)
        offset_z = random.uniform(-size/3, size/3)
        sphere_size = size * random.uniform(0.4, 0.6)
        
        glPushMatrix()
        glTranslatef(offset_x, offset_y, offset_z)
        glutSolidSphere(sphere_size, 16, 16)
        glPopMatrix()
...
def main():
    ...
    # Initialize tree positions (only once when the game first loads)
    initialize_trees()
    initialize_clouds()  # Initialize cloud positions
